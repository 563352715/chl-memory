...
| ~~M16~~ | **DONE 2026-05-13 (Phase 1 scaffold) — Highway.com client + research artifact (CHL commit 98b4316).** New `backend/highway_client.py` mirrors mercury_client architecture (vault-first token + 5-min TTL cache + 401 bust + circuit-breaker + 1h lookup cache). `_normalize()` maps Highway raw → CHL canonical (identity_status / rightful_owner_validated / is_dispatch_service / coi_on_file / active_alerts / double_broker_risk). 17 tests pass. Endpoints `GET /api/highway/status` + `GET /api/highway/lookup/{mc}`. **Pre-launch posture:** `_configured()=False` until operator subscribes to Connect Core ($500/mo) + drops `HIGHWAY_API_TOKEN` into Vault. Phase 2 = wire into `carrier_vetting.vet_carrier_for_load` fan-out (severity-mapping policy in research artifact, NOT YET applied). |
| ~~M17~~ | **DONE 2026-05-13 — SuperDispatch + CentralDispatch adapter stubs shipped (CHL commit fc4c761).** New `_AutoTransportBase` class in `loadboards.py` with vault-first credential pattern. `SuperDispatchAdapter` (key=super_dispatch, $250/mo API add-on, OAuth client-creds at developer.superdispatch.com) + `CentralDispatchAdapter` (key=central_dispatch, $214.95/mo Premium for Fulfillment + Event APIs, token endpoint id.centraldispatch.com/connect/token). Both registered in `ADAPTERS[]`. Pre-subscription posture: `configured` returns False until operator subscribes + drops token into Vault. setup_instructions includes step-by-step onboarding per the research-agent artifact. Phase 2 wiring (live API calls) waits for subscriptions. |
...
| ~~Stage1b-P6~~ | **DONE 2026-05-13 — pnl_monitor floor enforcement integration test (CHL commit 3c9d34d). FLOOR GAP CLOSED.** Prior hardblock pattern `factor_` (underscore) was missing `factors.reconciler` / `factors.dispute_handler` (dot package paths) — same gap on `mercury_` vs `mercury.client`. Stage 1b could have autonomously cron-restarted factor code paths. Expanded pattern set to cover both shapes. 9 tests including cartesian product across all source × severity, case-insensitivity, substring match, regression guards. 39 Stage 1b tests pass total. |
...
| ~~M9~~ | **DONE 2026-05-13 (CHL commit cac5b28) — DeliveryReceipt mounted in LoadDetailModal.** Conditional render when load.status in {booked, in_transit, delivered} OR load.bol_uploaded_at, AND a driver/carrier contact is present (driver_phone / driver_email / carrier_phone / carrier_email). Surfaces between AutoRepostPanel and details grid. |
...
| ~~M11~~ | **DONE 2026-05-13 — Operator-tunable settings UI (Phase 1+2+3 all shipped).** Phase 1: `GET /api/admin/settings` aggregates 5 storage shapes (untyped/singleton/global/key-singleton/id-default). Phase 2: `PUT /api/admin/settings/flag/{key}` writes to native doc-shape via SAFE_FLAGS whitelist (5 flags) with hardblock guards + audit. Phase 3: inline ON/OFF toggle buttons in admin dashboard. CHL commits a17e50c + 3138241. |
